...
def get_camera_pixel_directions(image_shape,intrinsics,normalized_intrinsics=True):
	...
	pts = get_coordinate_grid(image_shape[2:4], device=intrinsics.device)
	if normalized_intrinsics:
		pts = pixel_coords_to_normalized_coords(pts,image_shape[2:4])
	dirs = pixel_coords_to_directions(pts.repeat(image_shape[0],1,1,1),intrinsics)
	return dirs
